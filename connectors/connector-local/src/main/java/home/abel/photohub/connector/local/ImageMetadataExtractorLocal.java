...
public class ImageMetadataExtractorLocal  {
	//private static final long serialVersionUID = 4774888237028696522L;	
	final Logger logger = LoggerFactory.getLogger(ImageMetadataExtractorLocal.class);
	 ...
	 * @throws Exception
	 */
	public ImageMetadataExtractorLocal(File inputFile) throws Exception {
		...
		try {
//			this.gpsDir  = metadata.getDirectory(GpsDirectory.class);
//			this.subIFDD  = metadata.getDirectory(ExifSubIFDDirectory.class);
//			this.subIFDDDescr  = new ExifSubIFDDescriptor(subIFDD);
//			this.IFD0 = metadata.getDirectory(ExifIFD0Directory.class);
	...
	public BasePhotoMetadata loadMetadata(BasePhotoMetadata theMetadata) throws Exception{
		...
		if (gpsDir!= null ) {
			...
			else logger.debug("Image  :" +inputFile.getName()+ "  Gps data empty.");
		}
		else {
			logger.warn("Image :" +inputFile.getName()+ "  metaDir GpsDirectory is empty");
		}
		if ( subIFDD != null ) {
			...
			String tt = subIFDD.getString(ExifSubIFDDirectory.TAG_DATETIME_ORIGINAL);

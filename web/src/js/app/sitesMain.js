    ...
    $(document).ready(function() {
        ...
        function fillSiteForm(model) {
            try {
                ...
                //---------------------------------------------------------------------
                //  Уонвертируем в читабельный формат занятое место
                //---------------------------------------------------------------------
                $("#site_total").text(Utils.size2h(model.get('sizeTotal')));
                $("#site_space_limit").text(Utils.size2h(model.get('sizeLimit')));
                ...
                // Инициализируем объект  Actions  дял текущего сайта (используется для удаления или очистки)
